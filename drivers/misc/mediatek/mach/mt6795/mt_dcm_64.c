...
#define DCM_TAG     "[DCM] "
#define dcm_err(fmt, args...)		pr_err(DCM_TAG fmt, ##args)
#define dcm_warn(fmt, args...)		pr_warn(DCM_TAG fmt, ##args)
#define dcm_info(fmt, args...)		pr_warn(DCM_TAG fmt, ##args)
#define dcm_debug(fmt, args...)	    pr_debug(DCM_TAG fmt, ##args)
...
void dcm_dump_regs(unsigned int type)
{
	...
	if (type & IFR_DCM) {
		...
		dcm_info("[IFR_DCM]MM_MMU_DCM_DIS        :0x%08x, (0x%08x)\n", MM_MMU_DCM_DIS,
			 dcm_readl(MM_MMU_DCM_DIS));
		dcm_info("[IFR_DCM]PERISYS_MMU_DCM_DIS   :0x%08x, (0x%08x)\n", PERISYS_MMU_DCM_DIS,
			 dcm_readl(PERISYS_MMU_DCM_DIS));
	}
	...
	if (type & PER_DCM) {
		...
		volatile unsigned int i2c0_i2creg_hw_cg_en, i2c1_i2creg_hw_cg_en,
		    i2c2_i2creg_hw_cg_en, i2c3_i2creg_hw_cg_en, i2c4_i2creg_hw_cg_en;
	...
	if (1) {		/*type & VEN_DCM) { */
		if (1) {	/*subsys_is_on(SYS_ISP)) { */
			volatile unsigned int lv_VENC_CLK_CG_CTRL, lv_VENC_CLK_DCM_CTRL,
			    lv_JPGENC_DCM_CTRL, lv_JPGDEC_DCM_CTRL;
	...
	if (type & ISP_DCM) {
		if (1) {	/*subsys_is_on(SYS_ISP)) { */
			volatile unsigned int lv_CTL_RAW_DCM_DIS, lv_CTL_RAW_D_DCM_DIS,
			    lv_CTL_DMA_DCM_DIS, lv_CTL_RGB_DCM_DIS, lv_CTL_YUV_DCM_DIS,
			    lv_CTL_TOP_DCM_DIS;
...
void dcm_enable(unsigned int type)
{
	...
	if (type & CPU_DCM) {
		...
		dcm_setl(BUS_FABRIC_DCM_CTRL, 0x00011F0F);	/*0xF0200668,set bit0,bit1,bit2,bit3,
								   bit8,bit9,bit10,bit11,bit12,bit16=1 */
#endif
		dcm_sta |= CPU_DCM;
	}
...
#endif
		/*set bit0,bit1,bit8,bit9=1,DCM debouncing counter=0, */
		dcm_setl(INFRA_GLOBALCON_DCMCTL, 0x00000303);	/*0x10001050*/
	...
	if (type & PER_DCM) {
		...
		disable_clock(MT_CG_PERI_MSDC30_3, "DCM");
		 ...
		 /*USB*/ enable_clock(MT_CG_PERI_USB1, "DCM");
		dcm_clrl(USB0_DCM, 0x00070000);	/*0x11200700//clear bit16~bit18=0, */
		disable_clock(MT_CG_PERI_USB1, "DCM");

		 /*PMIC*/ dcm_setl(PMIC_WRAP_DCM_EN, 0x00000001);	/*0x1000D13C//set bit0=1, */
	...
	if (type & ISP_DCM) {	/*video encoder : sensor=>ISP=>VENC */
		...
		dcm_info
		    ("[%s][ISP_DCM     ]=0x%08x, subsys_is_on(SYS_ISP)=%d, subsys_is_on(SYS_VEN)=%d\n",
...
void dcm_disable(unsigned int type)
{
	...
	if (type & CPU_DCM) {
		...
		dcm_clrl(BUS_FABRIC_DCM_CTRL, 0x00011F0F);	/*0xF0200668,set bit0,bit1,bit2,bit3,bit8,bit9,bit10,
								   bit11,bit12,bit16=0 */
	...
	if (type & IFR_DCM) {
		...
		/*should off DRAMC first than off TOP_DCMCTL */
		 /*DRAMC*/ dcm_setl(channel_A_DRAMC_PD_CTRL, 0x01000000);	/*0xF00041DC,set bit24=1, */
...
/*USB0@0x11200000*/
	...
	if (node) {
		...
		dcm_info("[DCM_USB0] dcm_USB0_base=0x%p\n", dcm_USB0_base);
	} else {
		dcm_info("[DCM_USB0] dcm_USB0_base error\n");
...
/*MSDC0@0x11230000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MSDC0] dcm_MSDC0_base=0x%p\n", dcm_MSDC0_base);
	} else {
		dcm_info("[DCM_MSDC0] dcm_MSDC0_base error\n");
...
/*MSDC1@0x11240000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MSDC1] dcm_MSDC1_base=0x%p\n", dcm_MSDC1_base);
	} else {
		dcm_info("[DCM_MSDC1] dcm_MSDC1_base error\n");
...
/*MSDC2@0x11250000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MSDC2] dcm_MSDC2_base=0x%p\n", dcm_MSDC2_base);
	} else {
		dcm_info("[DCM_MSDC2] dcm_MSDC2_base error\n");
...
/*MSDC3@0x11260000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MSDC3] dcm_MSDC3_base=0x%p\n", dcm_MSDC3_base);
	} else {
		dcm_info("[DCM_MSDC3] dcm_MSDC3_base error\n");
...
/*PMIC_WRAP@0x1000D000*/
	...
	if (node) {
		...
		dcm_info("[DCM_PMIC_WRAP] dcm_PMIC_WRAP_base=0x%p\n", dcm_PMIC_WRAP_base);
	} else {
		dcm_info("[DCM_PMIC_WRAP] dcm_PMIC_WRAP_base error\n");
...
/*I2C0@0x11007000*/
	...
	if (node) {
		...
		dcm_info("[DCM_I2C0] dcm_I2C0_base=0x%p\n", dcm_I2C0_base);
	} else {
		dcm_info("[DCM_I2C0] dcm_I2C0_base error\n");
...
/*I2C1@0x11008000*/
	...
	if (node) {
		...
		dcm_info("[DCM_I2C1] dcm_I2C1_base=0x%p\n", dcm_I2C1_base);
	} else {
		dcm_info("[DCM_I2C1] dcm_I2C1_base error\n");
...
/*I2C2@0x11009000*/
	...
	if (node) {
		...
		dcm_info("[DCM_I2C2] dcm_I2C2_base=0x%p\n", dcm_I2C2_base);
	} else {
		dcm_info("[DCM_I2C2] dcm_I2C2_base error\n");
...
/*I2C3@0x11010000*/
	...
	if (node) {
		...
		dcm_info("[DCM_I2C3] dcm_I2C3_base=0x%p\n", dcm_I2C3_base);
	} else {
		dcm_info("[DCM_I2C3] dcm_I2C3_base error\n");
...
/*I2C4@0x11011000*/
	...
	if (node) {
		...
		dcm_info("[DCM_I2C4] dcm_I2C4_base=0x%p\n", dcm_I2C4_base);
	} else {
		dcm_I2C4_base = 0;
		dcm_info("[DCM_I2C4] dcm_I2C4_base error\n");
...
/*MJC_CONFIG@0x17000000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MJC_CONFIG] dcm_MJC_CONFIG_base=0x%p\n", dcm_MJC_CONFIG_base);
	} else {
		dcm_info("[DCM_MJC_CONFIG] dcm_MJC_CONFIG_base error\n");
...
/*MCUCFG@0x10200000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MCUCFG] dcm_MCUCFG_base=0x%p\n", dcm_MCUCFG_base);
	} else {
		dcm_info("[DCM_MCUCFG] dcm_MCUCFG_base error\n");
...
/*CKSYS@0x10000000*/
	...
	if (node) {
		...
		dcm_info("[DCM_TOPCKGEN] dcm_TOPCKGEN_base=0x%p\n", dcm_TOPCKGEN_base);
	} else {
		dcm_info("[DCM_TOPCKGEN] dcm_TOPCKGEN_base error\n");
...
/*INFRACFG_AO@0x10001000*/
	...
	if (node) {
		...
		dcm_info("[DCM_INFRACFG_AO] dcm_INFRACFG_AO_base=0x%p\n", dcm_INFRACFG_AO_base);
	} else {
		dcm_info("[DCM_INFRACFG_AO] dcm_INFRACFG_AO_base error\n");
...
/*M4U@0x10205000*/
	...
	if (node) {
		...
		dcm_info("[DCM_M4U] dcm_M4U_base=0x%p\n", dcm_M4U_base);
	} else {
		dcm_info("[DCM_M4U] dcm_M4U_base error\n");
...
/*PERISYS_IOMMU@0x10214000*/
	...
	if (node) {
		...
		dcm_info("[DCM_PERISYS_IOMMU] dcm_PERISYS_IOMMU_base=0x%p\n",
			 dcm_PERISYS_IOMMU_base);
	} else {
		dcm_info("[DCM_PERISYS_IOMMU] dcm_PERISYS_IOMMU_base error\n");
...
/*PERICFG@0x10003000*/
	...
	if (node) {
		...
		dcm_info("[DCM_PERICFG] dcm_PERICFG_base=0x%p\n", dcm_PERICFG_base);
	} else {
		dcm_info("[DCM_PERICFG] dcm_PERICFG_base error\n");
...
/*DRAMC0@0x10004000*/
	...
	if (node) {
		...
		dcm_info("[DCM_DRAMC0] dcm_DRAMC0_base=0x%p\n", dcm_DRAMC0_base);
	} else {
		dcm_info("[DCM_DRAMC0] dcm_DRAMC0_base error\n");
...
/*DRAMC1@0x10011000*/
	...
	if (node) {
		...
		dcm_info("[DCM_DRAMC1] dcm_DRAMC1_base=0x%p\n", dcm_DRAMC1_base);
	} else {
		dcm_info("[DCM_DRAMC1] dcm_DRAMC1_base error\n");
...
/*EMI@0x10203000*/
	...
	if (node) {
		...
		dcm_info("[DCM_EMI] dcm_EMI_base=0x%p\n", dcm_EMI_base);
	} else {
		dcm_info("[DCM_EMI] dcm_EMI_base error\n");
...
/*SMI_COMMON@0x14022000*/
	...
	if (node) {
		...
		dcm_info("[DCM_SMI_COMMON] dcm_SMI_COMMO_base=0x%p\n", dcm_SMI_COMMO_base);
	} else {
		dcm_info("[DCM_SMI_COMMON] dcm_SMI_COMMO_base error\n");
...
/*SMI_LARB0@0x14021000*/
	...
	if (node) {
		...
		dcm_info("[DCM_SMI_LARB0] dcm_SMI_LARB0_base=0x%p\n", dcm_SMI_LARB0_base);
	} else {
		dcm_info("[DCM_SMI_LARB0] dcm_SMI_LARB0_base error\n");
...
/*SMI_LARB1@16010000*/
	...
	if (node) {
		...
		dcm_info("[DCM_SMI_LARB1 dcm_SMI_LARB1_base=0x%p\n", dcm_SMI_LARB1_base);
	} else {
		dcm_info("[DCM_SMI_LARB1 dcm_SMI_LARB1_base error\n");
...
/*SMI_LARB2@0x15001000*/
	...
	if (node) {
		...
		dcm_info("[DCM_SMI_LARB2] dcm_SMI_LARB2_base=0x%p\n", dcm_SMI_LARB2_base);
	} else {
		dcm_info("[DCM_SMI_LARB2] dcm_SMI_LARB2_base error\n");
...
/*SMI_LARB3@0x18001000*/
	...
	if (node) {
		...
		dcm_info("[DCM_SMI_LARB3] dcm_SMI_LARB3_base=0x%p\n", dcm_SMI_LARB3_base);
	} else {
		dcm_info("[DCM_SMI_LARB3] dcm_SMI_LARB3_base error\n");
...
/*SMI_LARB5@0x17002000*/
	...
	if (node) {
		...
		dcm_info("[DCM_SMI_LARB5] dcm_SMI_LARB5_base=0x%p\n", dcm_SMI_LARB5_base);
	} else {
		dcm_info("[DCM_SMI_LARB5] dcm_SMI_LARB5_base error\n");
...
/*CAM1@0x15004000*/
	...
	if (node) {
		...
		dcm_info("[DCM_CAM1] dcm_CAM1_base=0x%p\n", dcm_CAM1_base);
	} else {
		dcm_info("[DCM_CAM1] dcm_CAM1_base error\n");
...
/*FDVT@0x1500B000*/
	...
	if (node) {
		...
		dcm_info("[DCM_FDVT] dcm_FDVT_base=0x%p\n", dcm_FDVT_base);
	} else {
		dcm_info("[DCM_FDVT] dcm_FDVT_base error\n");
...
/*JPGENC@0x18003000*/
	...
	if (node) {
		...
		dcm_info("[DCM_JPGENC] dcm_JPGENC_base=0x%p\n", dcm_JPGENC_base);
	} else {
		dcm_info("[DCM_JPGENC] dcm_JPGENC_base error\n");
...
/*JPGDEC@0x18004000*/
	...
	if (node) {
		...
		dcm_info("[DCM_JPGDEC] dcm_JPGDEC_base=0x%p\n", dcm_JPGDEC_base);
	} else {
		dcm_info("[DCM_JPGDEC] dcm_JPGDEC_base error\n");
...
/*MMSYS_CONFIG@0x14000000*/
	...
	if (node) {
		...
		dcm_info("[DCM_MMSYS_CONFIG] dcm_MMSYS_CONFIG_base=0x%p\n", dcm_MMSYS_CONFIG_base);
	} else {
		dcm_info("[DCM_MMSYS_CONFIG] dcm_MMSYS_CONFIG_base error\n");
...
/*VENC_GCON@0x18000000*/
	...
	if (node) {
		...
		dcm_info("[DCM_VENC] dcm_VENC_base=0x%p\n", dcm_VENC_base);
	} else {
		dcm_info("[DCM_VENC] dcm_VENC_base error\n");
...
/*VDEC_GCON@0x16000000*/
	...
	if (node) {
		...
		dcm_info("[DCM_VDEC_GCON] dcm_VDEC_GCON_base=0x%p\n", dcm_VDEC_GCON_base);
	} else {
		dcm_info("[DCM_VDEC_GCON] dcm_VDEC_GCON_base error\n");
...
/*E3TCM*/
	...
	if (node) {
		...
		dcm_info("[DCM_E3TCM] dcm_E3TCM_base=0x%p\n", dcm_E3TCM_base);
	} else {
		dcm_info("[DCM_E3TCM] dcm_E3TCM_base error\n");
